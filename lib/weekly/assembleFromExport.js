...
function buildRecommendations(userId, taps, usersById, nameFor, windowTaps = null) {
  ...
  // Build set of active users (users who had at least one tap in the time window)
  const activeUsers = new Set();
  let normalizedWindowTaps = [];
  
  if (windowTaps) {
    normalizedWindowTaps = windowTaps.map(tap => {
      const a = tap.user1_id || tap.id1;
      const b = tap.user2_id || tap.id2;
      return { a, b, time: tap.time };
    }).filter(tap => tap.a && tap.b && tap.a !== tap.b);
    
    normalizedWindowTaps.forEach(tap => {
      activeUsers.add(tap.a);
      activeUsers.add(tap.b);
    });
  }
  ...
  directConnections.forEach(directId => {
    ...
    if (neighbors) {
      neighbors.forEach(neighborId => {
        ...
        // Skip if user is not active in the time window (if windowTaps provided)
        if (windowTaps && !activeUsers.has(neighborId)) {
          return;
        }
        ...
        directConnections.forEach(otherDirectId => {
          ...
          if (otherNeighbors && otherNeighbors.has(neighborId)) {
            ...
            // Use windowTaps if provided, otherwise use all taps
            const tapsToUse = windowTaps ? normalizedWindowTaps : normalizedTaps;
            
            const myTapsWithFriend = tapsToUse.filter(tap => 
              (tap.a === userId && tap.b === otherDirectId) || 
              (tap.b === userId && tap.a === otherDirectId)
            ).length;
            
            const candidateTapsWithFriend = tapsToUse.filter(tap => 
            ...
            // Store mutual details with tap count and breakdown
            mutualDetails.push({
              id: otherDirectId,
              name: nameFor(otherDirectId),
              tapCount: mutualTapCount,
              youToMutualTaps: myTapsWithFriend,
              mutualToRecommendedTaps: candidateTapsWithFriend
            });
...
function assembleWeeklyFromExport({ userId, taps, users, nowUtc, timeWindow = '1week', isDebug = false }) {
  ...
  try {
    secondDegreeRecommendations = buildRecommendations(userId, taps, usersById, nameFor, windowTaps);
  } catch (e) {
    warnings.push(`recs_2nd_error:${e.message}`);
    secondDegreeRecommendations = [];
  }
  
  // 3rd degree recommendations disabled for now
  // try {
  ...
  // Combine recommendations with proper weighting (3rd degree disabled for now)
