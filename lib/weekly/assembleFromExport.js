...
function assembleWeeklyFromExport({ userId, taps, users, nowUtc }) {
  ...
  // Build name mapping from Data Reader export
  const toId = (u) => u.user_id || u.id;
  const fullName = (u) => [u.basic_info?.first_name, u.basic_info?.last_name].filter(Boolean).join(' ').trim();
  const username = (u) => u.basic_info?.username || u.username || '';
  const displayFromUser = (u) => fullName(u) || username(u) || 'Unknown';
  
  const userMap = new Map();
  for (const u of (users || [])) {
    const id = toId(u);
    if (id) userMap.set(id, displayFromUser(u));
  }
  ...
  const recommendations = buildRecommendations(userId, taps, usersById, nameFor);
