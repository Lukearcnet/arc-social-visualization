...
function calculateDegreeConnections(userId, windowTaps, allTaps, nameFor) {
  ...
  // Build historical 1st degree connections (all time) to exclude from all degrees
  ...
  // 1st Degree: All distinct others directly tapped with userId in windowTaps, excluding historical connections
  ...
  windowTaps.forEach(tap => {
    ...
    if (id1 === userId || id2 === userId) {
      const otherId = id1 === userId ? id2 : id1;
      if (otherId && otherId !== userId && !historicalFirstDegree.has(otherId)) {
