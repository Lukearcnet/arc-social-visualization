...
function calculateExpandedReach(userId, windowPairs, firstSeenAt, startTime, endTime, nameFor) {
  ...
  windowPairs.forEach(key => {
    ...
    if (firstSeen >= startTime && firstSeen < endTime) {
      const [a, b] = key.split('|');
      // Both users in this pair made a "new" connection
    ...
    meta: {
      ...
      debug: {
        ...
        leaderboard_comparison: {
          ...
          new_connections_top3: newConnectionsLeaderboard.slice(0, 3).map(x => ({ name: x.name, count: x.new_first_degree })),
          expanded_reach_top3: expandedReachTop.slice(0, 3).map(x => ({ name: x.name, count: x.net_new }))
        }
      }
    }
  };
