...
function buildRecommendations(userId, taps, usersById) {
      ...
      neighbors.forEach(neighborId => {
        ...
        directConnections.forEach(otherDirectId => {
          ...
          if (otherNeighbors && otherNeighbors.has(neighborId)) {
            mutualCount++;
            const user = usersById.get(otherDirectId);
            if (user) {
              const displayName = getDisplayName(user);
              if (!mutualNames.includes(displayName)) {
                mutualNames.push(displayName);
              }
            }
          }
        });
        
        // Only include candidates with ≥2 mutual connections
        if (mutualCount >= 2) {
  ...
  candidatesArray.sort((a, b) => {
    ...
    const nameA = getDisplayName(usersById.get(a.id)) || '';
    const nameB = getDisplayName(usersById.get(b.id)) || '';
    return nameA.localeCompare(nameB);
  });
  
  // Return top 3 candidates in the expected format
  return candidatesArray.slice(0, 3).map(candidate => ({
    user_id: candidate.id,
    name: getDisplayName(usersById.get(candidate.id)),
    scores: { total: candidate.score },
    mutuals: candidate.mutualNames,
...
function assembleWeeklyFromExport({ userId, taps, users, nowUtc }) {
  ...
  // Build user lookup map (export uses user_id, not id)
  const usersById = new Map(
    (users || []).map(u => [u.user_id || u.id, u])
  );
  
  // Helper to get display name from both root and basic_info keys
  const pick = (u, key) => (u?.[key] ?? u?.basic_info?.[key] ?? null);
  
  const getDisplayNameWithPick = (user) => {
    if (!user) return 'Unknown';
    const first = pick(user, 'first_name');
    const last = pick(user, 'last_name');
    ...
    if (otherId && otherId !== userId) {
      ...
      if (!existing || new Date(tap.time) > new Date(existing.last_tap_at)) {
        firstDegreeMap.set(otherId, {
          user_id: otherId,
          name: getDisplayNameWithPick(usersById.get(otherId)),
  ...
  const newConnectionsLeaderboard = Object.entries(otherPartyCounts)
    ...
    .map(([id, count]) => {
      ...
      return {
        user_id: id,
        name: getDisplayNameWithPick(user),
  ...
  // 6. Recommendations (mutual connections algorithm)
  const recommendations = buildRecommendations(userId, taps, usersById);
