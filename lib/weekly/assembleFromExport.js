...
const userById = (users, id) => safeUsers(users).find(u => u.id === id || u.user_id === id);
...
function assembleWeeklyFromExport({ userId, taps, users, nowUtc }) {
  try {
    ...
    // Safe name lookup
    const nameFor = (id) => nameOf(users, id);
  ...
  const payload = {
    ...
    meta: {
      ...
      warnings: warnings,
      debug: {
        users_mapped: safeUsers(users).length
      }
    }
  };
  ...
  return payload;
    ...
    // Return error payload with debug info
    return {
      recap: {
        first_degree_new: [],
