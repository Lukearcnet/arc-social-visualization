...
function calculateDegreeConnections(userId, windowTaps, allTaps, nameFor) {
  // Build adjacency map from allTaps for 2nd/3rd degree, windowTaps for 1st degree
  ...
  // 1st Degree: All distinct others directly tapped with userId in windowTaps (unchanged)
  ...
  // Now uses allTaps adjacency map but only shows connections made in time window
    ...
    if (neighbors) {
      neighbors.forEach(neighborId => {
        if (neighborId !== userId && !firstDegreeSet.has(neighborId)) {
          // Check if this connection was made in the time window
          const hasConnectionInWindow = windowTaps.some(tap => {
            const id1 = tap.user1_id || tap.id1;
            const id2 = tap.user2_id || tap.id2;
            return (id1 === firstDegreeId && id2 === neighborId) || 
                   (id1 === neighborId && id2 === firstDegreeId);
          });
          
          if (hasConnectionInWindow) {
      ...
      neighbors.forEach(neighborId => {
        if (neighborId !== userId && 
          ...
          const hasConnectionInWindow = windowTaps.some(tap => {
            const id1 = tap.user1_id || tap.id1;
            const id2 = tap.user2_id || tap.id2;
            return (id1 === secondDegreeId && id2 === neighborId) || 
                   (id1 === neighborId && id2 === secondDegreeId);
          });
